...
write.csv(SeaScape_cumsum_raw, "Write_Files/SeaScape_cumsum_raw.csv")
...
# ttt <- utils::unstack(df_long, amt_ng_ptnorm_smooth ~ Compound.Name)
...
cluster_all <- M_t_analyte_withIS_smoothed.zscore %>%
  left_join(clustered_data_tidy) %>% 
  filter(!is.na(cluster)) %>% 
  dplyr::mutate(cluster = ifelse(cluster == 7, 2, cluster)) %>% 
...
colsb = c("#CC6677", "#661100",  "#669900", "#336600",  "#9966FF","#660066","#9933CC")

cols2 = c("#CC6677", "#661100", "#CC3333",  "#9966FF","#660066","#9933CC", "#669900", "#336600")
cols2b = c("#CC6677", "#661100",  "#9966FF","#660066","#9933CC", "#669900", "#336600")
...
dev.new(width = 4, height = 4)
plot(1:n, pch = 16, cex = 2, col = colsb)
```
...
png("bioind12.png", width=6, height=14,
    units="in", res=600, pointsize=12)
bioi
dev.off()
```
...
# note need to finish updating the figures

bio.cor.info <- read_csv("Read_Files/correlation_inspection_ed4_virus_hb_chla.csv")%>% 
  left_join(cluster_assignments) %>% 
  left_join(h.clust.factors) %>% 
  left_join(SeaScape_normalized_cumsum.q) %>% 
  left_join(compiled_comp_info) 
...
write.csv(bio.cor.info, "Write_Files/correlation_info_inspection4.csv")
```
...
png("bio_cor_plot6.png", width=5.5, height=3.5,
    units="in", res=600, pointsize=12)
bio.c.plot
dev.off()
...
Making pretty figure of biological activity

```{r}

bio_spec.phyto <- bio_spec %>% 
  filter(!is.na(Diatom_Dominated_Aggregates)) %>% 
  dplyr::select("r_date", "Diatom_Dominated_Aggregates", "Diatoms", "Microzooplankton", 
                "Mixed_Aggregates", "Dinoflagellates", "Haptophytes") %>% 
  dplyr::mutate(Other_Phyto = Mixed_Aggregates+ Dinoflagellates+Haptophytes) %>% 
  dplyr::select("r_date", "Diatom_Dominated_Aggregates", "Diatoms", "Microzooplankton", 
                "Other_Phyto") %>% 
  dplyr::mutate(tot_Phyto = Diatom_Dominated_Aggregates + Diatoms +
                  Microzooplankton + Other_Phyto)

tot_Phyto.max <- max(bio_spec.phyto$tot_Phyto)

bio_spec.phyto.n <- bio_spec.phyto%>% 
  dplyr::mutate(DDA_norm = Diatom_Dominated_Aggregates/tot_Phyto.max) %>% 
  dplyr::mutate(Diatoms_norm = Diatoms/tot_Phyto.max) %>%
  dplyr::mutate(Mzoo_norm = Microzooplankton/tot_Phyto.max) %>% 
  dplyr::mutate(Other_norm = Other_Phyto/tot_Phyto.max) %>%
  dplyr::select("r_date", "DDA_norm", "Diatoms_norm", "Mzoo_norm", "Other_norm") %>% 
  gather(key = "phyto_cat", value = "normalized_conc", -r_date)
  

phyto.p <- bio_spec.phyto.n %>% 
  ggplot()+
  geom_col(aes(x = r_date, y = normalized_conc, fill = phyto_cat))+
  geom_line(data = bio_spec, aes(x = r_date, y = HB_cells_L/max(bio_spec$HB_cells_L),
                                 color = "HB"), size = 2)+
  geom_line(data = bio_spec, aes(x = r_date, y = Virus_L/max(bio_spec$Virus_L), 
                                 color = "Viruses"), size = 2)+
  geom_line(data = bio_spec, aes(x = r_date, y = Chlor_a/max(bio_spec$Chlor_a),
                                 color= "Chlorophyll-a"), size = 2)+
  scale_fill_manual(name = "Phytoplankton\nPhenotype", 
                    labels = c("Diatom Dominates\nAggregates", "Diatoms", 
                               "Microzooplankton", "Other"), 
                    values = c("grey80", "grey 60", "grey 40", "grey20"))+
  scale_color_manual(name = "Other Bloom Indicators", 
                     labels = c("Chlorophyll-a", "Heterotrophic\nBacteria", "Viruses"),
                     values = c("green3", "blue2", "red4"))+
  theme_bw(base_size = 14)+
  ylab("Normalized Concentration in Water")+
  xlab("")

phyto.p

png("phyto.p.5.png", width=8, height=5,
    units="in", res=600, pointsize=12)
phyto.p
dev.off()
png()
...
carbon_pool <- M_t_analyte_withIS.hclust %>% 
  ...
  scale_fill_manual(values = colsb[1:7])+
...
carbon_pool2 <- M_t_analyte_withIS.hclust %>% 
  ...
  ggtitle("Submicron Sea Spray Aerosol \n Carbon Pool Dynamics")+
  scale_fill_manual(values = cols2b[1:7], name = "")+
  theme(plot.title = element_text(size = 18, hjust = .5), legend.text = element_text(size = 14), legend.title=element_text(size=14), axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 14), axis.title.y = element_text(size = 16))+
  scale_x_datetime(date_breaks = "6 days", labels = date_format("%b %d"))
...
png("carbon_pool5.png", width=6, height=4.5,
    units="in", res=600, pointsize=12)
carbon_pool2
dev.off()
...
carbon_pool_nodiiso <- M_t_analyte_withIS.hclust %>% 
  ...
  ylab("Mass Fraction (Recovered)")+
  ggtitle("Submicron Sea Spray Aerosol \n Carbon Pool Dynamics")+
  scale_fill_manual(values = colsb[1:7], name = "")+
  theme(plot.title = element_text(size = 18, hjust = .5), legend.text = element_text(size = 14), legend.title=element_text(size=14), axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 14), axis.title.y = element_text(size = 16))+
  scale_x_datetime(date_breaks = "6 days", labels = date_format("%b %d"))
...
png("carbon_pool_forsi2.png", width=6, height=4.5,
    units="in", res=600, pointsize=12)
cp.si
dev.off()
...
day.f1 <- M_t_analyte_withIS.hclust %>% 
  filter(Filter_num == "UCB19_A_153" |Filter_num == "UCB19_A_156" ) %>% 
  filter(!is.na(cluster_group)) %>% 
  group_by(cluster_group) %>% 
  summarise(tot_mass = sum(amt_ng_per_m3_air)) %>% 
...
day.l1 <- M_t_analyte_withIS.hclust %>% 
  filter(Filter_num == "UCB19_A_196" |Filter_num == "UCB19_A_197" ) %>% 
  filter(!is.na(cluster_group)) %>% 
  group_by(cluster_group) %>% 
  summarise(tot_mass = sum(amt_ng_per_m3_air)) %>% 
...
oil_s <- read_csv("Read_Files/Oil-Compounds.csv")

oil.t <- M_t_analyte_withIS.hclust %>% 
  filter(Filter_num == "UCB19_A_153" |Filter_num == "UCB19_A_156" ) %>% 
  left_join(oil_s) %>% 
  group_by(cluster_factor_types, is_oil) %>% 
  summarise(mass.s = sum(amt_ng_per_m3_air))
...
suns_s <- read_csv("Read_Files/Sunscreen_cpnds.csv")

suns.t <- M_t_analyte_withIS.hclust %>% 
  filter(Filter_num == "UCB19_A_153" |Filter_num == "UCB19_A_156" ) %>% 
  left_join(suns_s) %>% 
  group_by(cluster_factor_types, is_suns) %>% 
  summarise(mass.s = sum(amt_ng_per_m3_air))

suns.t2 <- M_t_analyte_withIS.hclust %>% 
  filter(Filter_num == "UCB19_A_153" |Filter_num == "UCB19_A_156" ) %>% 
  left_join(suns_s) %>% 
  group_by(cluster_group, is_suns) %>% 
  summarise(mass.s = sum(amt_ng_per_m3_air))

suns.t3 <- M_t_analyte_withIS.hclust %>% 
  filter(Filter_num == "UCB19_A_153" |Filter_num == "UCB19_A_156" ) %>% 
  left_join(suns_s) %>% 
  group_by(is_suns) %>% 
  summarise(mass.s = sum(amt_ng_per_m3_air, na.rm = TRUE))

pah_s <- read_csv("Read_Files/PAH_compounds.csv")

pah.t <- M_t_analyte_withIS.hclust %>% 
  filter(Filter_num == "UCB19_A_153" |Filter_num == "UCB19_A_156" ) %>% 
  left_join(pah_s) %>% 
  group_by(cluster_factor_types, is_PAH) %>% 
  summarise(mass.s = sum(amt_ng_per_m3_air))

pah.t2 <- M_t_analyte_withIS.hclust %>% 
  #filter(Filter_num == "UCB19_A_153" |Filter_num == "UCB19_A_156" ) %>% 
  left_join(pah_s) %>% 
  filter(is_PAH == TRUE) #%>% 
  #dplyr::select(Compound.Name, cluster_group)
  # group_by(cluster_group, is_PAH) %>% 
  # summarise(mass.s = sum(amt_ng_per_m3_air))

pah.t2 %>% 
  ggplot(aes(x = r_date, y = amt_ng_per_m3_air, color = Compound.Name))+
  geom_line()

pah.t3 <- M_t_analyte_withIS.hclust %>% 
  filter(Filter_num == "UCB19_A_153" |Filter_num == "UCB19_A_156" ) %>% 
  left_join(pah_s) %>% 
  filter(is_PAH == TRUE) %>% 
  group_by(name) %>% 
...
Comp_info_prop %>% 
  ...
  scale_color_manual(values = cols2b[1:7], name = "")
...
png("properties_trial2.png", width=7, height=4.5,
    units="in", res=600, pointsize=12)
prop.l1
dev.off()
...
png("proptiles2.png", width=7, height=10,
    units="in", res=600, pointsize=12)
prop.tiles
dev.off()
